(define (domain vincentland)
    ...
    (:action escort
        ...
        :effect (and (at ?p ?locB) (at ?charA ?locB) (not (at ?p ?locA)) (not (at ?charA ?locA)) (increase (total-cost) 2)))
		
	(:action exchange
		:parameters (?p ?char ?i2 ?i1 ?loc)
		:precondition (and (player ?p) (or (character ?char) (monster ?char)) (item ?i1) (item ?i2) (location ?loc) (cooperative ?char) (has ?p ?i1) (has ?char ?i2) (at ?p ?loc) (at ?char ?loc))
	...
	(:action giveto
		:parameters (?p ?charB ?i ?loc)
   		...
   		:effect (and (has ?charB ?i) (not (has ?p ?i)) (cooperative ?charB) (increase (total-cost) 1)))

	(:action move
		:parameters (?p ?to ?from)
		:precondition (and (location ?to) (location ?from) (player ?p) (at ?p ?from))
		:effect (and (at ?p ?to) (not (at ?p ?from)) (increase (total-cost) 3)))
